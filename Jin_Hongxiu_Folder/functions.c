...
void xiugai(yezhu* current)
{
	...
	while (1)
	{
		...
		if (b[0] == 'Y')
		{
			...
			tmp = checkadd2(add1, add2, add3, current);//检查地址是否重复（传current避免和现有地址重复）
			if (tmp == 1)
			{
				printf("错误，该地址已被注册\a\n");
				continue;
	...
	while (1)
	{
		...
		if (b[0] == 'Y')
		{
			printf("请输入\n");
			scanf("%s", name);
			int tmp1 = checkname2(name);//// 检查用户名长度(1-20)
			if (tmp1 == 1)
			{
				continue;
			}
			int tmp2 = checkname(name);//查重名（用户名）
			if (tmp2 == 0)
			{
...
void getcurrentdate(char* dateStr)// 获取当前系统日期
{
	...
	struct tm* t = localtime(&now);//时间戳解析为 struct tm(年月日)，localtime 是 C 语言标准库中的时间转换函数，用于将日历时间（时间戳）转换为本地时间的 tm 结构体。
	//将年月日复制到char*里
...
void print(fee* p) {
	char tmp[10];
	strcpy(tmp, strcmp(p->yn, "y") == 0 ? "已缴费" : "未缴费");//是y打印已缴费，不是打印未缴费
	char pay_date[20];
	if (strcmp(p->yn, "y") == 0)
	{
...
// 1.按时间精确查询（支持账单日期/缴费日期，包括年月日，必须按格式输入）
void shijian_jingque(char* shijian)//传入的是从在菜单时输入的日期
{
	...
	while (p != NULL)
	{
			...
			strcmp(p->add3, current->add3) == 0)
		{
			...
			if (comparedate(bianli, mubiao) == 1)//比较日期（年月日）
			{
				print(p);
				count++;
			}
...
void cx_by_zhuangtai_shijian_jine(char* zhuangtai, char* rq, char* je)//在菜单时输入了状态，日期，金额
{
	...
	while (p) {
		if (!strcmp(p->add1, current->add1) && !strcmp(p->add2, current->add2) && !strcmp(p->add3, current->add3) && !strcmp(p->yn, zhuangtai)) //找到对应用户，且匹配状态，状态是在菜单的时候输入的
		{
			// 缴费日期查询过滤未缴费记录
			if (query_type == 2 && strcmp(p->yn, "n") == 0) {
				p = p->next1;
...
void yijiao()
{
	...
	while (p != NULL)
	{
			...
			strcmp(p->add3, current->add3) == 0)//找到对应用户
		{
			...
			{
				count++;
				total += atoi(p->sum);//atoi是字符串转数字函数
			}
		}
		p = p->next1;
	}
